...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Install the packaged zaomeng skill into OpenClaw, Hermes Agent, or a local project skills directory"
    )
    ...
    if args.openclaw_dir:
        target = copy_skill_file(packaged_src, Path(args.openclaw_dir), "zaomeng-skill")
        print(f"Installed OpenClaw skill to: {target}")

    if args.hermes_dir:
        target = copy_skill_file(packaged_src, Path(args.hermes_dir), "zaomeng-skill")
        print(f"Installed Hermes skill to: {target}")

    if args.skills_dir:
        target = copy_skill_dir(packaged_src, Path(args.skills_dir), args.skill_name)
        print(f"Installed generic skill to: {target}")

    if args.project_root:
        project_skills_root = Path(args.project_root) / "skills"
        target = copy_skill_dir(packaged_src, project_skills_root, args.skill_name)
