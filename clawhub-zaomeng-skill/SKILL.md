...
- 在 OpenClaw 这类已经自带模型能力的宿主里，`local-rule-engine` 不是阻塞条件；不要因为运行时没单独填写 `runtime/config.yaml` 就停下来让用户二选一。
- 如果宿主本身已经配置了模型或可用的本地 Ollama，就直接继续执行用户请求；不要提示“去配置 runtime/config.yaml 才能继续群聊”。
...
- 群聊 / 单聊请求一旦已经满足 `chat --message` 的执行条件，就直接执行，不要插入“配置 LLM（推荐）/继续规则模式”这类额外选择题。
...
- 不要在 OpenClaw 中因为看到 `llm.provider=local-rule-engine` 就要求用户去改 `runtime/config.yaml`、补 API Key，或在“配置 LLM”和“继续规则模式”之间做选择。
...
```text
有两个方案：
配置 LLM（推荐）：在 runtime/config.yaml 里填入 OpenAI 或 Claude 的 API Key
继续当前规则模式：我继续推进多轮对话
你想试哪个？
```
