...
- OpenClaw 自身如果已经配置模型能力，这就是可用宿主能力；不要再把“是否单独配置 zaomeng 运行时 LLM”当成继续群聊的前置条件。
...
- 只要已经能执行 `chat --message`，就直接继续，不要向用户抛出“去填 runtime/config.yaml”或“配置 LLM / 继续规则模式”的二选一提示。
- 首选用法：
...
- 不要因为运行时默认是 `local-rule-engine` 就让用户停下来配置额外 API Key；这在 OpenClaw 场景下不是必需步骤。
